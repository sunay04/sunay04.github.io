...
function SiteEditor({ page, site, onChange }: { page: Exclude<EditorPageKey, "projects">; site: SiteContent; onChange: (site: SiteContent) => void }) {
  ...
  if (page === "about") return <div className="editor-site-fields"><SectionHeading title="关于页面" /><Field label="主标题" value={site.profile.heroTitle} onChange={(heroTitle) => onChange({ ...site, profile: { ...site.profile, heroTitle } })} /><Field label="教育信息（每行一项）" multiline value={site.profile.education.join("\n")} onChange={(value) => onChange({ ...site, profile: { ...site.profile, education: value.split("\n").filter(Boolean) } })} /></div>;
