...
function MusicEditor({ tracks, onChange, onUpload }: { tracks: MusicTrack[]; onChange: (tracks: MusicTrack[]) => void; onUpload: (file: File) => Promise<string> }) {
...
export function EditorPage() {
  ...
  const uploadMedia = async (file: File) => {
    const maxSize = file.type.startsWith("audio/") ? 20 * 1024 * 1024 : 8 * 1024 * 1024;
    if (file.size > maxSize) throw new Error(`单个${file.type.startsWith("audio/") ? "音频" : "媒体"}文件不能超过 ${file.type.startsWith("audio/") ? 20 : 8} MB`);
  ...
  return <main className="editor-shell">
    ...
    <div className="editor-workspace" data-mode={mode} data-area={activePage === "projects" ? "projects" : "site"}>
      ...
      <section className="editor-canvas"><div className="editor-document">
        ...
        {activePage === "music" && <MusicEditor tracks={site.music ?? bundledSite.music} onChange={(music) => setSite({ ...site, music })} onUpload={uploadMedia} />}
