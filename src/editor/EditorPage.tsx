...
function PropertyPicker({ label, options, values, colors, multiple, onChange, onColorChange }: { label: string; options: string[]; values: string[]; colors: Record<string, string>; multiple?: boolean; onChange: (values: string[]) => void; onColorChange: (value: string, color: string) => void }) {
  ...
  return <div className="editor-property"><span className="editor-property-label">{label}</span><div className="editor-property-control"><div className="editor-property-value"> <button className="editor-property-trigger" aria-expanded={open} onClick={() => setOpen((value) => !value)}>{values.length ? values.map((value) => <span className="editor-property-chip" style={{ backgroundColor: colors[value] ?? "#e4e7eb" }} key={value}>{value}</span>) : <span className="editor-property-empty">选择或创建</span>}<ChevronRight size={14} /></button>{values.map((value) => <label className="editor-color-control" title={`设置${value}的颜色`} key={value}><input type="color" value={colors[value] ?? "#dce2e8"} onChange={(event) => onColorChange(value, event.target.value)} /><span style={{ backgroundColor: colors[value] ?? "#dce2e8" }} /></label>)}</div>{open && <div className="editor-property-popover"><input autoFocus value={query} placeholder={`搜索或创建${label}`} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Escape") setOpen(false); if (event.key === "Enter" && createValue) { event.preventDefault(); choose(createValue); setQuery(""); } }} /><div className="editor-property-options">{matches.map((option) => <button key={option} aria-pressed={values.includes(option)} onClick={() => choose(option)}><span className="editor-property-dot" style={{ backgroundColor: colors[option] ?? "#9aa7b1" }} />{option}{values.includes(option) && <Check size={14} />}</button>)}{createValue && !options.includes(createValue) && <button onClick={() => { choose(createValue); setQuery(""); }}><Plus size={14} />创建“{createValue}”</button>}</div></div>}</div></div>;
}
